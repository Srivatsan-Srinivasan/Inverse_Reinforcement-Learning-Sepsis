...
def Q_learning_solver_for_irl(task, transition_matrix, reward_matrix, NUM_STATES, NUM_ACTIONS, episode_count = 500, max_task_iter = np.inf, epsilon = 0.2):
    ...
    for episode_iter in range( episode_count ):
        ...
        if iteration >= 1000 and episode_iter >= 100:
            break
        else:
            # Start the task 
            task.reset()
...
def Q_value_iteration(transition_matrix, reward_matrix, verbose, theta=1e-3, gamma=0.99):
    '''
    ...
    for t in itertools.count():
        ...
        Q = transition_matrix.dot(reward_matrix + gamma * v_old)
        # test = reward_matrix + gamma * v_old
        ...
        v_old = v
    if verbose:
        print ("Number of rounds for Q value to converge:", t)
    return Q
