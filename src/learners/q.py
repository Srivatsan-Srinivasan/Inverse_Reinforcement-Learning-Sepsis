...
def update_Q_Qlearning(Q_table, alpha, gamma, state, action, reward, new_state):
    ...
    best_actions = np.argwhere(Q_table[new_state,:] == np.max(Q_table[new_state,:]))
    best_action = np.random.choice(best_actions.flatten().tolist())
    td_target = reward + gamma * Q_table[new_state, best_action]
    td_delta = td_target - Q_table[state, action]    
    Q_table[state, action] += alpha * td_delta
    return Q_table 

def q_train(env, hyperparams, trial_count=1, episode_min_count=100, global_min_iter_count=5000, local_max_iter_count=100):        
